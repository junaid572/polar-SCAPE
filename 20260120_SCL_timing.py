...
if __name__ == '__main__':
    ...
    ns = range(7, 14)          # n = 8..13
    runs = 100                # number of repetitions
    err_est = 0.1              # fixed channel estimate
    freq = 1
    ...
    for n in ns:
        print(f"Running n = {n} (N = {2**n}) ...")

        N = 2**n
        kk = 2**(n-1)

        # Generate time-varying Pauli channel probabilities
        probs, _ = TVQC_prob(
    ...
    plt.errorbar(N_values, construct_mean, yerr=construct_std,
                 fmt='-o', label='Construct')
    plt.errorbar(N_values, encode_mean, yerr=encode_std,
                 fmt='-o', label='Encode')
    plt.errorbar(N_values, decode_mean, yerr=decode_std,
                 fmt='-o', label='Decode')

    plt.xscale('log', base=2)
    #plt.yscale('log')

    plt.xlabel("Blocklength N = 2^n")
    plt.ylabel("Time (seconds)")
    plt.title("Polar Code Timing vs Blocklength (mean ± std over 1000 runs)")
    ...
    # ----------------------------------------------------
    filename = "polar_timing_results.txt"
